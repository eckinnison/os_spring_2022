...
devcall uartPutc(device *devptr, char c)
{
    ...
    if(oidle == true){
        oidle = false;
        // write the character to the UART control and status register's data register.
    }
    else if(oidle == false){
        // use the appropriate UART semaphore to wait for the UART's buffer space to be free
        // Next, place the character into the UART output buffer with respect to the index of the buffer's first byte, amount of bytes in buffer, and total length of the UART output buffer.
        // Then, increment the counter of bytes in the output buffer. 
    }
